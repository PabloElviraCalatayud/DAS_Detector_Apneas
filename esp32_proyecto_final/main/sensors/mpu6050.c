...
// === I2C funciones básicas ===
static esp_err_t i2c_write(uint8_t reg, uint8_t data) {
  uint8_t buf[2] = {reg, data};
  return i2c_master_write_to_device(s_i2c_port, MPU6050_ADDR, buf, sizeof(buf), pdMS_TO_TICKS(100));
}

static esp_err_t i2c_read(uint8_t reg, uint8_t *out, size_t len) {
  return i2c_master_write_read_device(s_i2c_port, MPU6050_ADDR, &reg, 1, out, len, pdMS_TO_TICKS(100));
}
...
static void mpu_task(void *arg) {
  ...
  while (1) {
    if (i2c_read(MPU6050_REG_ACCEL_XOUT_H, raw, sizeof(raw)) == ESP_OK) {
      ...
      // Corrige intercambio de ejes (X ↔ Y) si el diseño lo requiere
      float ax_corr = ay_f;
      float ay_corr = ax_f;
      float az_corr = az_f;
      float gx_corr = gy_f;
...
esp_err_t mpu6050_start(i2c_port_t i2c_port, gpio_num_t sda, gpio_num_t scl, uint32_t clk_hz,
  ...
  // Despertar sensor
  esp_err_t err = i2c_write(MPU6050_REG_PWR_MGMT1, 0x00);
  if (err != ESP_OK) {
    ESP_LOGE(TAG, "No se pudo iniciar MPU6050");
    return err;
  }
  ...
  BaseType_t ok = xTaskCreate(mpu_task, "mpu6050_task", stack_size ? stack_size : 4096,
                              NULL, task_prio ? task_prio : 5, &s_task);
  if (ok != pdPASS) {
    ESP_LOGE(TAG, "No se pudo crear tarea MPU");
    return ESP_FAIL;
  }

  ESP_LOGI(TAG, "MPU6050 inicializado en I2C%d (SDA=%d, SCL=%d)", s_i2c_port, sda, scl);
  return ESP_OK;
}
...
esp_err_t mpu6050_calibrate(size_t samples) {
  ...
  ESP_LOGI(TAG, "Calibrado: ax=%d ay=%d az=%d gx=%d gy=%d gz=%d",
           s_offset_ax, s_offset_ay, s_offset_az, s_offset_gx, s_offset_gy, s_offset_gz);
  return ESP_OK;
}
